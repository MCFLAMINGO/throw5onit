var __viem__ = (() => {
  ...
  var init_hashSignature = __esm({
    "node_modules/viem/_esm/utils/hash/hashSignature.js"() {
      init_toBytes();
      init_keccak256();
      hash = (value) => keccak256(toBytes3(value));
    }
  });
...
${prettyStateOverride(stateOverride)}`;
  ...
  function encodeArg({ param, value }) {
    if (param.type === "string" || param.type === "bytes")
      return keccak256(toBytes3(value));
    ...
    function isEqual(input, value, arg) {
      try {
        ...
        if (input.type === "string" || input.type === "bytes")
          return keccak256(toBytes3(value)) === arg;
        return value === arg;
      } catch {
        return false;
      }
  ...
  function namehash(name) {
    ...
    for (let i = labels.length - 1; i >= 0; i -= 1) {
      ...
      const hashed = hashFromEncodedLabel ? toBytes3(hashFromEncodedLabel) : keccak256(stringToBytes(labels[i]), "bytes");
